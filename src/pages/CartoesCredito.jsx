    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto animate-fade-in pb-24">
      <header className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2">Gerencie limites e feche faturas automaticamente</p>
        </div>
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-500 flex justify-center"><Loader2 className="animate-spin w-8 h-8"/></div>
      ) : cartoes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {cartoes.map((cartao) => {
            ...
            return (
              <div key={cartao.id} className="relative group bg-white rounded-3xl p-4 shadow-soft border border-slate-100 flex flex-col h-full">
                ...
                <div className="mt-4 px-2 flex-1 flex flex-col justify-end">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex-1 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl font-medium text-sm transition-colors flex items-center justify-center gap-2 border border-slate-200 shadow-sm"
                    >
                    ...
                    <button 
                      ...
                      className="flex-1 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-xl font-medium text-sm transition-colors flex items-center justify-center gap-2 border border-blue-200 shadow-sm disabled:opacity-50 disabled:grayscale"
                    >
            ...
            <div className="p-6 overflow-y-auto">
              <form onSubmit={handleSalvarNovo} className="space-y-4">
                <div>
                  ...
                  <input type="text" required value={novoCartao.nome} onChange={(e) => setNovoCartao({...novoCartao, nome: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Ex: Nubank, Itaú..." />
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Limite Total (R$)</label>
                    <input type="number" step="0.01" required value={novoCartao.limite} onChange={(e) => setNovoCartao({...novoCartao, limite: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="0.00" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Fatura Atual (R$)</label>
                    <input type="number" step="0.01" value={novoCartao.fatura_atual} onChange={(e) => setNovoCartao({...novoCartao, fatura_atual: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Opcional" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" required value={novoCartao.dia_fechamento} onChange={(e) => setNovoCartao({...novoCartao, dia_fechamento: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Ex: 5" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Dia de Vencimento</label>
                    <input type="number" required value={novoCartao.dia_vencimento} onChange={(e) => setNovoCartao({...novoCartao, dia_vencimento: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Ex: 10" />
                  </div>
                </div>
      ...
      {modalGasto.show && createPortal(
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm flex flex-col">
            ...
            <div className="p-6">
              <p className="text-sm text-slate-500 mb-4">Adicionando gasto no cartão <strong className="text-slate-800">{modalGasto.cartao?.nome}</strong></p>
              <form onSubmit={confirmarGasto} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Valor da Compra (R$)</label>
      ...
      {modalEditar.show && createPortal(
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm flex flex-col">
            ...
            <div className="p-6">
              <p className="text-sm text-slate-500 mb-4">Atualizando o limite do <strong className="text-slate-800">{modalEditar.cartao?.nome}</strong></p>
              <form onSubmit={confirmarEdicao} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Novo Limite Total (R$)</label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <input autoFocus type="number" step="0.01" required value={modalEditar.limite} onChange={(e) => setModalEditar({...modalEditar, limite: e.target.value})} className="w-full pl-9 pr-4 py-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-semibold" placeholder="0.00" />
                  </div>
                </div>
                <button type="submit" disabled={salvando} className="w-full py-3.5 bg-gradient-to-r from-blue-900 to-cyan-500 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all flex justify-center items-center gap-2">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm flex flex-col">
            ...
            <div className="p-6 text-center">
              ...
              <h2 className="font-bold text-xl text-slate-800 mb-2">Confirma o Fechamento?</h2>
              <p className="text-slate-500 text-sm mb-6">
                O valor de <strong className="text-slate-800">{formatarMoeda(modalFechar.cartao?.fatura_atual)}</strong> será enviado para a tela de <strong>Contas a Pagar</strong>. <br/><br/>
                O limite deste cartão continuará bloqueado até que você registre o pagamento na outra tela.
              </p>
              
              <div className="flex gap-3">
                <button type="button" onClick={() => setModalFechar({ show: false, cartao: null })} className="flex-1 py-3 bg-slate-100 text-slate-700 rounded-xl font-semibold hover:bg-slate-200 transition-colors">
                  Cancelar
                </button>
